...
export class PortfolioHistoricalValueModel {
  ...
  static async findByDateRange(
    ...
    return await db
      ...
      .where(and(
        ...
        between(portfolioHistoricalValues.date, startDate, endDate)
      ))
      .orderBy(asc(portfolioHistoricalValues.date));
  }
  ...
  static async recordTodayValue(
    ...
    const [existingRecord] = await db
      ...
      .where(and(
        eq(portfolioHistoricalValues.portfolioId, portfolioId),
        eq(portfolioHistoricalValues.date, today),
        eq(portfolioHistoricalValues.timeframe, timeframe)
      ));
  ...
  static async calculatePerformance(
    ...
    const [startValue] = await db
      ...
      .where(and(
        eq(portfolioHistoricalValues.portfolioId, portfolioId),
        eq(portfolioHistoricalValues.date, startDate)
      ))
      .limit(1);
    ...
    if (!startValue) {
      ...
      const [nearestValue] = await db
        ...
        .where(and(
          eq(portfolioHistoricalValues.portfolioId, portfolioId),
          sql`${portfolioHistoricalValues.date} <= ${startDate}`
        ))
        .orderBy(desc(portfolioHistoricalValues.date))
        .limit(1);
      ...
      return {
        ...
        historical: await this.findByDateRange(portfolioId, nearestValue.date, endDate)
      };
    }
    ...
    return {
      ...
      historical: await this.findByDateRange(portfolioId, startDate, endDate)
    };
  }
}
