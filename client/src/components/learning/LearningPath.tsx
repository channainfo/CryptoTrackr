...
export const LearningPath: React.FC<LearningPathProps> = ({ 
  ...
  const achievementLevel = 
    progressPercentage >= 100 ? 'diamond' :
    progressPercentage >= 80 ? 'platinum' :
    progressPercentage >= 60 ? 'gold' :
    progressPercentage >= 40 ? 'silver' :
    progressPercentage >= 20 ? 'bronze' :
              ...
              return (
                <div key={module.id} className="relative flex items-start gap-4">
                  ...
                  <div className="z-10 mt-1">
                    <GemBadge 
                      tier={isCompleted ? 'completed' : isInProgress ? 'in_progress' : 'not_started'} 
                      showLabel={false}
                      size="lg"
                      animate
                    />
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start mb-1">
                      <h3 className={`font-semibold ${isCompleted ? 'text-blue-600 dark:text-blue-400' : ''}`}>
                        {module.title}
                      </h3>
                      <span className={`text-sm ${isCompleted ? 'text-green-600 dark:text-green-400' : 'text-gray-500 dark:text-gray-400'}`}>
