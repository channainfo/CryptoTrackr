...
import { RefreshCw, TrendingUp, BarChart3, ChevronDown, ChevronUp, RotateCw, AlertCircle } from 'lucide-react';
import { 
  Card, 
  CardContent, 
  CardDescription, 
...
export default function PortfolioRebalance({ portfolioId }: PortfolioRebalanceProps) {
  ...
  const { data: assets, isLoading, refetch } = useQuery<PortfolioAsset[]>({
  ...
  const getSuggestions = (): RebalanceSuggestion[] => {
    ...
    const getBtcAllocation = () => {
      switch(riskLevel) {
        case 1: return 0.40; // Conservative: 40% BTC
        case 2: return 0.45;
        case 3: return 0.50; // Balanced: 50% BTC
        case 4: return 0.55;
        case 5: return 0.60; // Aggressive: 60% BTC
        default: return 0.50;
      }
    };
    ...
    const getEthAllocation = () => {
      switch(riskLevel) {
        case 1: return 0.30; // Conservative: 30% ETH
        case 2: return 0.30;
        case 3: return 0.30; // Balanced: 30% ETH
        case 4: return 0.30;
        case 5: return 0.30; // Aggressive: 30% ETH
        default: return 0.30;
      }
    };
    ...
    const otherAssets = assets.filter((asset: PortfolioAsset) => 
    ...
    const suggestions = assets.map((asset: PortfolioAsset) => {
      ...
      const amountCrypto = asset.currentPrice ? amountUsd / asset.currentPrice : 0;
    ...
    return suggestions.filter(s => 
      Math.abs(s.targetAllocation - s.currentAllocation) > 0.01
  ...
  const getBalanceScore = (): number => {
    ...
    const totalDeviation = suggestions.reduce((sum, s) => 
      sum + Math.abs(s.targetAllocation - s.currentAllocation), 0
    );
  ...
  const getBalanceHealth = (): { status: string; color: string } => {
    if (balanceScore >= 90) return { status: 'Excellent', color: 'text-green-500' };
    if (balanceScore >= 75) return { status: 'Good', color: 'text-emerald-500' };
    if (balanceScore >= 60) return { status: 'Fair', color: 'text-amber-500' };
    if (balanceScore >= 40) return { status: 'Needs Attention', color: 'text-orange-500' };
  ...
  const executeRebalance = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Your portfolio rebalance orders have been placed successfully.",
      });
  ...
  if (!assets || assets.length < 2) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Portfolio Balance</CardTitle>
          <CardDescription>Add more assets to see rebalance suggestions</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="h-40 flex flex-col items-center justify-center text-center space-y-3">
            <AlertCircle className="h-8 w-8 text-muted-foreground" />
            <p className="text-muted-foreground">
              Your portfolio needs at least 2 different cryptocurrencies for rebalancing suggestions.
            </p>
          </div>
        </CardContent>
      </Card>
          ...
          <div className="flex justify-between items-center mb-1">
            ...
            <Badge variant="outline">
              {riskLevel === 1 ? 'Conservative' : 
               riskLevel === 2 ? 'Moderately Conservative' :
               riskLevel === 3 ? 'Balanced' :
               riskLevel === 4 ? 'Growth' : 'Aggressive'}
            </Badge>
          </div>
          <Slider 
            value={[riskLevel]} 
          ...
          <>
            <div className="flex justify-between items-center mb-3">
              ...
              <AlertDialog open={showRebalanceConfirm} onOpenChange={setShowRebalanceConfirm}>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    <AlertDialogTitle>Confirm Portfolio Rebalance</AlertDialogTitle>
                    <AlertDialogDescription>
                      This will place {suggestions.length} orders to rebalance your portfolio.
                  ...
                  {suggestions.map((suggestion, index) => (
                    <div key={index} className="border-t pt-3 first:border-t-0 first:pt-0">
                      ...
                      <div className="text-xs text-muted-foreground mt-1">
                        {suggestion.action === 'buy' ? 'Buy' : 'Sell'} approximately {suggestion.amountCrypto.toFixed(6)} {suggestion.symbol}
                      </div>
                    </div>
                  ))}
                </div>
