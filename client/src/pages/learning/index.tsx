...
import {
  ModuleSkeleton,
  ModuleSkeletonList,
} from "@/components/learning/ModuleSkeleton";
...
const ModuleCard = ({
  ...
  const progressValue =
    status === "completed"
      ? 100
      : status === "in_progress"
        ? (progress?.find((p) => p.moduleId === module.id)
            ?.lastCompletedSection || 0) * 20
        : 0;
...
const LearningPage = () => {
  ...
  const modulesToDisplay = category === "all" ? allModules : categoryModules;
  ...
  useEffect(() => {
    if (simulatedProgress.length > 0) {
      localStorage.setItem(
        ...
        JSON.stringify(simulatedProgress),
      );
    }
  }, [simulatedProgress]);

  // Combine server progress with simulated progress
  const effectiveProgress = userProgress?.length
    ? userProgress
    : simulatedProgress;
  ...
  const effectiveStats = React.useMemo(() => {
    ...
    if (
      ...
      allModules.length > 0 &&
      effectiveProgress &&
      effectiveProgress.length > 0
    ) {
      const totalModules = allModules.length;
      const completedModules = effectiveProgress.filter(
        (p) => p.status === "completed",
      ).length;
      const inProgressModules = effectiveProgress.filter(
    ...
    <Container>
      <div className="py-8">
          ...
          <Card className="mb-8 learning-stats">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="p-4 border rounded-lg">
                  <div className="text-2xl font-bold">
                    {effectiveStats.completedModules}
                  </div>
                  <div className="text-sm text-muted-foreground">Completed</div>
                </div>
                <div className="p-4 border rounded-lg">
                  <div className="text-2xl font-bold">
                    {effectiveStats.inProgressModules}
                  </div>
                ...
                <div className="p-4 border rounded-lg">
                  <div className="text-2xl font-bold">
                    {effectiveStats.notStartedModules}
                  </div>
                  <div className="text-sm text-muted-foreground">
                    Not Started
                  </div>
                ...
                <div className="p-4 border rounded-lg">
                  <div className="text-2xl font-bold">
                    {effectiveStats.completionPercentage}%
                  </div>
                  <div className="text-sm text-muted-foreground">
                    Overall Completion
                  </div>
              ...
              <div className="mt-4">
                ...
                <Progress
                  value={effectiveStats.completionPercentage}
                  className="h-2"
                />
              </div>
            </CardContent>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div>
                    ...
                    <Progress
                      value={effectiveStats?.completionPercentage || 0}
                      className="h-2"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="p-3 border rounded-lg text-center">
                      <div className="text-xl font-bold">
                        {effectiveStats?.completedModules || 0}
                      </div>
                    ...
                    <div className="p-3 border rounded-lg text-center">
                      <div className="text-xl font-bold">
                        {effectiveStats?.inProgressModules || 0}
                      </div>
                      <div className="text-xs text-muted-foreground">
                        In Progress
                      </div>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="flex items-center gap-2 text-sm text-muted-foreground mb-4">
                        <Clock className="w-4 h-4" />
                        <span>
                          {recommendedModule.estimatedMinutes} minutes
                        </span>
                        ...
                        >
                          <span className="mr-1">
                            {
                              CategoryIcons[
                                recommendedModule.category as LearningCategory
                              ]
                            }
                          </span>
                          {recommendedModule.category.charAt(0).toUpperCase() +
                            recommendedModule.category.slice(1)}
                        </Badge>
                      </div>
        ...
        <div className="border-t border-gray-200 dark:border-gray-800 my-12 pt-4">
          ...
          <Tabs
            defaultValue="all"
            className="w-full learning-tabs"
        ...
        steps={[
          {
            ...
            content:
              "This is your learning dashboard. Here you can see your progress across all learning modules.",
            disableBeacon: true,
          },
          {
            target: ".learning-path",
            content:
              "The learning path shows your journey through different crypto topics. Follow it to build your knowledge systematically.",
            disableBeacon: true,
          },
          {
            target: ".learning-achievements",
            content:
              "As you complete modules, you'll earn achievements that showcase your growing expertise.",
            disableBeacon: true,
          },
          {
            target: ".learning-share",
            content:
              "Share your achievements on social media to showcase your crypto knowledge.",
            disableBeacon: true,
          },
          {
            target: ".learning-tabs",
            content:
              "Filter modules by category to focus on specific areas of interest. Start with the basics and work your way up to advanced topics.",
            disableBeacon: true,
          },
        ]}
      />
