...
import { searchDefinitions, getAllDefinitions } from "@/data/crypto-definitions";
...
const GlossaryPage = () => {
  ...
  const definitions = searchTerm ? searchDefinitions(searchTerm) : getAllDefinitions();
  const categorizedDefinitions = categorizeDefinitions(definitions);

  return (
    <div className="p-4 md:p-6 lg:p-8 pb-20 md:pb-8 max-w-5xl mx-auto">
      {/* Page Header */}
      <div className="flex flex-col mb-8">
        <div className="flex items-center space-x-3 mb-2">
          <BookOpen className="h-6 w-6 text-primary" />
          <h1 className="text-3xl font-bold dark:text-white">Crypto Glossary</h1>
        </div>
        <p className="text-neutral-mid dark:text-gray-400 mt-1 mb-6">
          Learn the essential cryptocurrency terms and concepts to better understand the crypto market.
        </p>
          ...
          {Object.entries(categorizedDefinitions).map(([category, defs]) => (
            <div key={category}>
              ...
              <div className="grid grid-cols-1 gap-4">
                {defs.map((def) => (
                  <Card key={def.term} className="dark:bg-zinc-900 border-0 shadow-sm">
                    <CardHeader className="pb-2">
                      <CardTitle className="text-lg font-semibold">{def.term}</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <p className="text-neutral-mid dark:text-gray-400">{def.definition}</p>
                    </CardContent>
                    {def.learnMoreUrl && (
                      <CardFooter>
                        <Button variant="link" className="p-0 h-auto text-primary" asChild>
                          <a href={def.learnMoreUrl} target="_blank" rel="noopener noreferrer">
                            Learn more →
                          </a>
                        </Button>
                      </CardFooter>
